		...
		void UpdateDebugInformation (Instruction removedInstruction, Instruction existingInstruction)
		{
			...
			if (custom_debug_infos != null) {
				foreach (var custom_debug_info in custom_debug_infos) {
					if (custom_debug_info is StateMachineScopeDebugInformation state_machine_scope) { UpdateStateMachineScope (state_machine_scope, ref resolver); }
					if (custom_debug_info is AsyncMethodBodyDebugInformation async_method_body) { UpdateAsyncMethodBody (async_method_body, ref resolver); }
